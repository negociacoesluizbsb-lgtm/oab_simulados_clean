...
function Checkout({ session, onActivate }: { session: UserSession | null; onActivate: () => void }) {
  ...
  return <div className='page narrow'><div className='card'><h2>Assinatura Premium</h2><p className='checkout-price'><strong>{PRICE}</strong></p><p className='checkout-copy'>Desbloqueie o banco completo de questões, continue o simulado além da amostra e estude com mais profundidade.</p><ul><li>Banco completo de questões</li><li>Simulado estendido</li><li>Acesso premium liberado</li></ul><button className='cta primary' onClick={start}>Assinar com Mercado Pago</button></div></div>;
}
...
function Simulado({ questoes, session }: { questoes: Questao[]; session: UserSession | null }) {
  ...
  return <div className='page'><div className='sectionhead'><div><h1>Simulado</h1><p>{isPremium ? 'Modo premium liberado.' : 'Você está vendo 1 questão de amostra do simulado.'}</p></div>{!isPremium && <Link href='/checkout' className='cta primary'>Desbloquear premium</Link>}</div>{q ? <QuestionCard question={q.pergunta} alternatives={q.alternativas} selectedAnswer={answer} correctAnswer={q.correta} showResult={checked} onAnswerClick={setAnswer} onSubmit={submit} onNext={next} isLastQuestion={i === list.length - 1} /> : <div className='card'><h3>Sem questões</h3></div>}{checked && i === list.length - 1 && !isPremium && <div className='card paywall'><h3>Fim da amostra</h3><p>Para continuar no simulado e acessar mais questões, desbloqueie o premium.</p><button className='cta primary' onClick={()=>nav('/checkout')}>Assinar para continuar</button></div>}</div>;
}
